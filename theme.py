...
def get_stylesheet(theme='dark'):
    """Returns the CSS stylesheet for the requested theme."""
    font_family = "'Segoe UI', sans-serif" if IS_WINDOWS else "sans-serif"
    
    light_theme = f"""
        QWidget {{ background-color: #f0f2f5; color: #1c1e21; font-family: {font_family}; }}
    ...
    dark_theme = f"""
        QWidget {{ background-color: #18191a; color: #e4e6eb; font-family: {font_family}; }}
        QTabWidget::pane {{ border: none; }}
...
def get_system_theme():
    """Detects the system theme (light/dark) for Windows, macOS, and Linux."""
    if IS_WINDOWS:
        try:
        ...
        try:
            ...
            # This command only succeeds in dark mode.
            return 'dark' if 'Dark' in result.stdout else 'light'
        except (FileNotFoundError, subprocess.CalledProcessError):
            # If the command fails, it means the key doesn't exist, which indicates light mode.
            return 'light'
    else: # For Linux
        try:
            ...
            return 'dark' # Default for Linux if command fails
